...
const STORAGE_BUCKET_REGISTRY = {
  ...
  'quarantine': {
    ...
    allowedMimeTypes: ['*/*'],
    maxSizeBytes: 50 * 1024 * 1024, // 50MB
    description: 'Isolated quarantine bucket for detected malware and suspicious files. Zero download access.'
  }
};
...
const {
  ...
  getFileScanMetadata,
  getStorageSecurityDashboardData
} = require('./malwareLedger');
...
async function validateAttachment(bucket, fileName, mimeType, fileBuffer) {
  ...
  if (!bucketConfig) {
    return {
      valid: false,
      error: `Invalid storage bucket: "${bucket}"`,
  ...
  // Run comprehensive malware and file validation scan (extension, magic bytes, exploits)
  const scan = scanFile(fileBuffer, fileName, mimeType);
  if (scan.scanResult === 'Quarantined') {
    return {
      valid: false,
      error: `Malware Security Alert: ${scan.reason}`,
  ...
  if (scan.scanResult === 'Suspicious') {
    return {
      valid: false,
      error: `Security Policy Warning: ${scan.reason}`,
      virusScanStatus: 'SUSPICIOUS',
      scanResult: 'Suspicious',
      threatType: scan.threatType,
      reason: scan.reason,
  ...
  if (!bucketConfig.allowedMimeTypes.includes(mimeType.toLowerCase()) && !bucketConfig.allowedMimeTypes.includes('*/*')) {
    return {
      valid: false,
