 ...
 * "Database schema mismatch detected. Administrator attention required."
 *
...
const USER_FACING_SCHEMA_ERROR = 'Database schema mismatch detected. Administrator attention required.';

const DEPLOYMENT_VERSION = process.env.DEPLOYMENT_VERSION || process.env.NEXT_PUBLIC_APP_VERSION || process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 7) || 'v1.0.0-prod';
...
function parsePrismaSchemaError(err, affectedRoute = 'Unknown') {
  ...
  console.error('[Schema Mismatch Detected]:', {
    ...
    'Affected Route': logEntry.affectedRoute,
    'Deployment Version': logEntry.deploymentVersion,
