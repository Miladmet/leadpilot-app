...
async function validateDatabaseSchema(prismaClient) {
  let schemaResult = await verifyDatabaseSchema(prismaClient);

  // If drift detected, attempt automatic self-healing before evaluating
  if (!schemaResult.isHealthy) {
    try {
      const { selfHealDatabaseSchema } = require('./dbSelfHealCore');
      await selfHealDatabaseSchema(prismaClient);
      schemaResult = await verifyDatabaseSchema(prismaClient);
    } catch (e) {}
  }
